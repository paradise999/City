	...
	{		
		...
		for (int i = 0; i < save.Length; i++) {
			...
			sql = "Insert Into properties (rot, coor, \"time\", side, red) Values (" + (int)save [i].transform.rotation.eulerAngles.y + ",'{"
				+ (int)save [i].transform.position.x + "," + (int)save [i].transform.position.y + "," + (int)save [i].transform.position.z + "}','{"+ (int)t.g+","+(int)t.r +"}',"+ save[i].GetComponent<road>().s +","+ t.red +")";
			com = new NpgsqlCommand (sql, conn);
			com.ExecuteNonQuery ();
		}
		...
		for (int i = 0; i < save.Length; i++) {
			sql = "Insert Into walls (rot, coor, scale) Values (" + (int)save [i].transform.rotation.eulerAngles.y + ",'{"
				+ (int)save [i].transform.position.x + "," + (int)save [i].transform.position.y + "," + (int)save [i].transform.position.z + "}','{"+ (int)save[i].transform.localScale.x +","+(int)save[i].transform.localScale.y +","+(int)save[i].transform.localScale.z +"}')";
			com = new NpgsqlCommand (sql, conn);
			com.ExecuteNonQuery ();
		}
